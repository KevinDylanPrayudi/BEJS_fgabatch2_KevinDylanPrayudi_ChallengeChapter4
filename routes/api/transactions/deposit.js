    ...
    async function post(req, res) {
        try {
            ...
            if (err instanceof Prisma.PrismaClientKnownRequestError) {
                if (err.code === 'P2003') {
                    return res.status(400).json(`The ${err.meta.field_name == 'Transactions_transaction_type_id_fkey (index)' ? 'transaction_type_id' : 'destination_account_id'} doesn't exists in other table.`);
                }
            }
